...
describe('testing rendering product', () => {
  ...
  test('rendering product Item', async () => {
    ...
    await waitFor(
      () => expect(screen.getAllByTestId('small-image')[0]).toBeInTheDocument(),
      { timeout: 6000 }
    );

    const image2 = screen.getAllByTestId('small-image');
    act(() => {
      fireEvent.click(image2[0]);
    });

    const image1 = screen.getByTestId('big-image');
    expect(image1.src).toEqual(image2[0].src);
  });
});
